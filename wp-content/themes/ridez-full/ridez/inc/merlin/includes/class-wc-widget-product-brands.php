...
class Ridez_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'ridez');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'ridez');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'ridez'),
				'label' => esc_html__('Title', 'ridez'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'ridez'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'ridez'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'ridez'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'ridez'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'ridez'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'ridez'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'ridez'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Ridez_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'ridez');
			...
			echo '<ul class="product-brands" id="ridez-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Ridez_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'ridez'),
		'singular_name'              => esc_html__('Brands', 'ridez'),
		'menu_name'                  => esc_html__('Brands', 'ridez'),
		'all_items'                  => esc_html__('All Brands', 'ridez'),
		'parent_item'                => esc_html__('Parent Brand', 'ridez'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'ridez'),
		'new_item_name'              => esc_html__('New Brand Name', 'ridez'),
		'add_new_item'               => esc_html__('Add New Brands', 'ridez'),
		'edit_item'                  => esc_html__('Edit Brand', 'ridez'),
		'update_item'                => esc_html__('Update Brand', 'ridez'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'ridez'),
		'search_items'               => esc_html__('Search Brands', 'ridez'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'ridez'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'ridez'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
